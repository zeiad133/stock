...
def update_stock_prices(stock):
  ...
  if not cached_stock: 
    ...
    price_today_stats =  price_today(price, parsed_time,cached_stock)
    price_this_hour_stats = price_this_hour(price, parsed_time,cached_stock)
    cache.set(stock_id,{
      'name': stock.get('name'),
      'availability': stock.get('availability'),
      'lowest_price_today': price_today_stats.get('lowest_price_today'),
      'highest_price_today': price_today_stats.get('highest_price_today'),
      'lowest_price_this_hour': price_this_hour_stats.get('lowest_price_this_hour'),
      'highest_price_this_hour': price_this_hour_stats.get('highest_price_this_hour'),
      'current_price': price,
      'current_day': parsed_time.day,
      'current_hour': parsed_time.hour
      })